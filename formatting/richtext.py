...
def _rebuild_from_kept(
    ...
    for row_index, (_start, _end, content, sep) in enumerate(rows):
        ...
        if (is_lightning_line(content) or is_lightning_heavy(content)) and not covered:
...
def _best_divider(mappings: list[EmojiMapping] | None) -> tuple[str, str] | None:
    ...
    for mapping in mappings or []:
        ...
        if not tagged and not inferred:
            continue
        ranked.append((1 if tagged else 0, int(mapping.priority or 0), mapping.unicode_emoji, str(mapping.custom_emoji_id)))
    if not ranked:
        return None
...
def apply_divider_library(
    ...
    """Turn a degraded ⚡ row, or a plain ━ rule, into one gold-line emoji.

    One captured divider must not be copied into two short bars. A run of
    rule lines, including two gold bars already on the post, collapses to one.
    """
    if not text:
        return text, marks, []
    chosen = _best_divider(mappings)
    ...
    while index < len(rows):
        if not _is_collapsible_line(rows[index][2]):
            index += 1
            continue
        end = index + 1
        while end < len(rows) and (_is_collapsible_line(rows[end][2]) or not rows[end][2].strip()):
            end += 1
        ...
        heavy = _is_rule_line(content)
        wholly = _whole_line_custom(rows[first][0], rows[first][1], marks)
        if len(rule_indexes) == 1 and (not heavy or wholly):
            index = end
            continue
        covered = _line_custom(rows[first][0], rows[first][1], marks)
        if len(rule_indexes) > 1 and covered and wholly and not is_lightning_heavy(content):
            fallback, custom_id = content.strip(), covered.custom_emoji_id
        elif chosen:
            fallback, custom_id = chosen
        else:
            fallback, custom_id = LONG_RULE, None
        replacements[first] = (fallback, custom_id)
    ...
    segments: list[tuple[int, int, str]] = []
    inserted: list[tuple[int, str, str | None]] = []
    new_cursor = 0
    for row_index, (old_start, old_end, content, sep) in enumerate(rows):
        ...
        if row_index in replacements:
            fallback, custom_id = replacements[row_index]
            ...
            if custom_id:
                inserted.append((new_cursor, fallback, custom_id))
    ...
    extra: list[Mark] = []
    for start, fallback, custom_id in inserted:
        if not custom_id:
            continue
        if start < 0 or start + len(fallback) > len(updated) or updated[start:start + len(fallback)] != fallback:
            continue
        extra.append(Mark("custom_emoji", start, start + len(fallback), custom_emoji_id=custom_id))
    if extra:
        kept = [
...
def append_missing_chrome(
    ...
    *,
    ...
    if add_divider and not has_divider:
        ...
        if dividers:
            fallback, custom_id = dividers[0]
            prefix = updated + "\n\n"
            updated = prefix + fallback
            mark_list.append(Mark("custom_emoji", len(prefix), len(prefix) + len(fallback), custom_emoji_id=custom_id))
...
def decorate_body(
    ...
    *,
    ...
    """Put real library emoji on the title and a few lines. Words stay."""
    ...
    if not content_indexes:
        return text, marks, []
    targets = [content_indexes[0]]
    if layout == "scatter" and len(content_indexes) > 2:
        targets.append(content_indexes[len(content_indexes) // 2])
    elif layout == "list":
        for index in content_indexes[1:]:
            if index not in targets:
                targets.append(index)
            if len(targets) >= min(4, max_insert):
                break
    elif layout == "closing":
        targets = [content_indexes[0]]
    targets = targets[:max_insert]
    updated = text
    current = list(marks)
    applied: list[str] = []
    for offset, index in enumerate(reversed(targets)):
        start, end, content, _sep = rows[index]
        if _line_starts_with_emoji(content) and layout != "closing":
            continue
        accent = pool[(len(targets) - 1 - offset) % len(pool)]
        emoji = accent.unicode_emoji
        if layout == "closing":
            prefix_at = end
            insert = " " + emoji
            mark_start = end + 1
